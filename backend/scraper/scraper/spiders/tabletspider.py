...
class TabletSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
             f'https://www.flanco.ro/telefoane-tablete/tablete-si-accesorii/tablete-',
             f'https://altex.ro/tablete/cpl/filtru/in-stoc-5182/in-stoc/brand-3334/{self.brand}/',
             f'https://flip.ro/magazin/{self.brand}/?modelType=Tablete',
             f'https://mediagalaxy.ro/tablete/cpl/filtru/in-stoc-5182/in-stoc/brand-3334/{self.brand}/',
             f'https://www.evomag.ro/telefoane-tablete-accesorii-tablete-si-accesorii-tablete/{self.brand}'
        ]
        ...
        for url in urls:
            if "altex.ro" in url:
                ...
                if "apple" in self.brand.lower():
                    yield scrapy.Request(url=url + "ios.html", callback=self.parse_flanco)
                else:
                    yield scrapy.Request(url=url + f"android/filtre/brand/{self.brand}.html",
                                         callback=self.parse_flanco)
            elif "flip.ro" in url:
        ...
        for products in response.css("li.Products-item"):
            try:
                ...
                if model in name.lower():
                    yield {
                        "name": name,
                        "price": re.search("[0-9]*[.,]?[0-9]+", price)[0],
            ...
            try:
                ...
                if model in name.lower():
                    yield {
                        "name": name,
                        "price": re.search("[0-9]*[.,]?[0-9]+", price)[0],
                        "link": "https://mediagalaxy.ro" + link,
                        "photo": photo
                    }
            except:
                name = products.css("span.Product-name::text").get()
    ...
    def parse_flanco(self, response):
        ...
        for products in response.css("li.item.product"):
            ...
            if model in name.lower():
                photo = re.sub('.*http', 'http',
                               products.css("a.product.photo.product-item-photo img::attr(src)").get())
                yield {
                    "name": name,
        ...
        for product in response.css("div.nice_product_container"):
            ...
            if model in title.lower():
                yield {
                    ...
                    "price": re.search("[0-9]*[.,]?[0-9]+", product.css("span.real_price::text").get())[0],
                    "photo": photo
                }
